...
public class PubSubConnection {
  ...
  public static final String SUBSCRIBE_KEY="subscribe_cluster_";
  ...
  public PubSubConnection(Socket socket,ReplicatedJedisPool jedisPool, String clusterNode) throws IOException {
    this.socket       = socket;
    this.outputStream = socket.getOutputStream();
    this.inputStream  = new RedisInputStream(new BufferedInputStream(socket.getInputStream()));
    this.closed       = new AtomicBoolean(false);
  ...
  public static String getSubscribeKey(String channelName) {
    return SUBSCRIBE_KEY+channelName;
  }
  ...
  public void subscribe(String channelName) throws IOException {
    ...
    if(jedisPool!=null) {
      String subKey = getSubscribeKey(channelName);
      try {
        jedisPool.getJedisCluster().set(subKey, clusterNode);
      }catch (Exception e){
        logger.error("PubSubConnection.subscribe.set error!",e.getMessage());
      }
    }
  }


  public void unsubscribe(String channelName,int statusCode) throws IOException {
    if (closed.get()) throw new IOException("Connection closed!");

    byte[] command = Util.combine(UNSUBSCRIBE_COMMAND, channelName.getBytes(), CRLF);
    outputStream.write(command);
//    if(jedisPool!=null&&statusCode!=1006) {
//      String subKey = getSubscribeKey(channelName);
//      try (Jedis jedis = this.jedisPool.getWriteResource(subKey)) {
//        jedis.del(subKey);
//      }
//    }
  }

...
public class DispatchManager extends Thread {
                         ...
                         Optional<DispatchChannel> deadLetterChannel)
  {
    ...
    SharedMetricRegistries.getOrCreate(Constants.METRICS_NAME)
            .register(name(DispatchManager.class, "DispatchManager_executor_depth"),
  ...
  public synchronized void unsubscribe(String name, DispatchChannel channel ,int statusCode) {
    Optional<DispatchChannel> subscription = Optional.ofNullable(subscriptions.get(name));
    ...
    if (subscription.isPresent() && subscription.get() == channel) {
      ...
      try {
        pubSubConnection.unsubscribe(name,statusCode);
      } catch (IOException e) {
        logger.warn("Unsubscribe error", e);
      }
  ...
  public void run() {
    while (running) {
      try {
        ...
        if (running) {
          this.pubSubConnection.close();
          logger.warn("PubSub Connection Error ! close cost:{}",System.currentTimeMillis()-time);
          time=System.currentTimeMillis();
          this.pubSubConnection = redisPubSubConnectionFactory.connect();
          logger.warn("***** PubSub reConnection ***** cost:{}",System.currentTimeMillis()-time);
          resubscribeAll();
        }
      }
    }

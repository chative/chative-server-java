...
public class DirectoryUpdater {
  ...
  public void updateFromLocalDatabase() {
    ...
    try {
      ...
      for (;;) {
        Map<String, Account> accounts = accountsManager.getAll(offset, CHUNK_SIZE);
        ...
        for (Account account : accounts.values()) {
          if (accountsManager.isActive(account)) {
            ...
            ClientContact clientContact = new ClientContact(token, null, accountsManager.isVoiceSupported(account), accountsManager.isVideoSupported(account));

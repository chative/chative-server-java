...
//    RedisOperation.unchecked(() -> apnFallbackManager.cancel(account, device));
    messagesManager.setSendConversationMsgLock(address.getNumber(),address.getDeviceId());
    ...
    context.addListener(new WebSocketSessionContext.WebSocketEventListener() {
      ...
      public void onWebSocketClose(WebSocketSessionContext context, int statusCode, String reason) {
        pubSubManager.unsubscribe(address, connection,statusCode);
        messagesManager.clearPriorConversation(address.getNumber(),address.getDeviceId());
        messagesManager.removeSendConversationMsgLock(address.getNumber(),address.getDeviceId());
        timer.stop();
      }
    });
  }

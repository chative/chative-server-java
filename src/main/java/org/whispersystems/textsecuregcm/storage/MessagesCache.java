...
public class MessagesCache implements Managed {
  ...
  private OutgoingMessageEntity constructEntityFromEnvelope(long id, Envelope envelope) {
    return new OutgoingMessageEntity(id, true,
                                     ...
                                     envelope.hasContent() ? envelope.getContent().toByteArray() : null,false,
                                     envelope.getSystemShowTimestamp(),envelope.getSequenceId(),envelope.getNotifySequenceId(),envelope.getMsgType().getNumber(),null,
            envelope.hasSourceIdentityKey() ? envelope.getSourceIdentityKey() : null,
            envelope.hasPeerContext() ? envelope.getPeerContext() : null);
  }
  ...
  private static class MessagePersister extends Thread {
    ...
    private void persistMessage(Key key, long score, byte[] message) {
      try {
        ...
        database.store(envelope, key.getAddress(), key.getDeviceId(),false,0,null,-1,
                envelope.hasSourceIdentityKey()?envelope.getSourceIdentityKey():null,
                envelope.hasPeerContext()?envelope.getPeerContext():null);
      } catch (InvalidProtocolBufferException e) {

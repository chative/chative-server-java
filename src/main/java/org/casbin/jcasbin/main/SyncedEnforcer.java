...
public class SyncedEnforcer extends Enforcer {
     ...
     * @param watcher the watcher.
     */
    public void setWatcherEx(Watcher watcher) {
        this.watcher = watcher;
    }
     ...
     * notice only load casbin memory
     * If the rule already exists, the function returns false and the rule will not be added.
     * Otherwise the function returns true by adding the new rule.
     *
     * @param params the "p" policy rule, ptype "p" is implicitly used.
    ...
    public boolean addMemoryPolicy(String... params) {
        ...
        try {
            ...
            var2 = super.addMemoryPolicy(params);
        } finally {
    ...
    public boolean removeMemoryPolicy(List<String> params) {
        try {
            READ_WRITE_LOCK.writeLock().lock();
            return super.removeMemoryPolicy(params);
        } finally {
            READ_WRITE_LOCK.writeLock().unlock();
        }
    }
    ...
    public boolean addGroupingMemoryPolicy(String... params) {
        try {
            READ_WRITE_LOCK.writeLock().lock();
            return super.addGroupingMemoryPolicy(params);
        } finally {
            READ_WRITE_LOCK.writeLock().unlock();
        }

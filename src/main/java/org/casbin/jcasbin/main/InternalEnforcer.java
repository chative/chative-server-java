...
class InternalEnforcer extends CoreEnforcer {
    ...
    boolean addMemoryPolicy(String sec, String ptype, List<String> rule) {
        if (this.mustUseDispatcher()) {
            this.dispatcher.addPolicies(sec, ptype, Collections.singletonList(rule));
            return true;
        } else if (this.model.hasPolicy(sec, ptype, rule)) {
            return false;
        } else {
    ...
    boolean addPolicy(String sec, String ptype, List<String> rule) {
        ...
        if (watcher != null && autoNotifyWatcher) {
            if (watcher instanceof WatcherEx) {
                ((WatcherEx) watcher).updateForAddPolicy(ptype, rule.toArray(new String[0]));
            } else {
                watcher.update();
            }
        }
    ...
    boolean removePolicy(String sec, String ptype, List<String> rule) {
        ...
        if (watcher != null && autoNotifyWatcher) {
            if (watcher instanceof WatcherEx) {
                ((WatcherEx) watcher).updateForRemovePolicy(ptype, rule.toArray(new String[0]));
            } else {
                watcher.update();
            }
        }
    ...
    boolean removeMemoryPolicy(String sec, String ptype, List<String> rule) {
        if (mustUseDispatcher()) {
            dispatcher.removePolicies(sec, ptype, Collections.singletonList(rule));
            return true;
        }
        ...
        buildIncrementalRoleLinks(sec, ptype, Collections.singletonList(rule), Model.PolicyOperations.POLICY_REMOVE);
        return true;
    }
    ...
    boolean updatePolicy(String sec, String ptype, List<String> oldRule, List<String> newRule) {
        ...
        if (watcher != null && autoNotifyWatcher) {
            try {
                if (watcher instanceof WatcherEx) {
                    ((WatcherEx) watcher).updateForUpdatePolicy(oldRule, newRule);
                } else {
                    watcher.update();
                }
            } catch (Exception e) {
     ...
     * @return succeeds or not.
     */
    boolean updateMemoryPolicy(String sec, String ptype, List<String> oldRule, List<String> newRule) {
        if (mustUseDispatcher()) {
            dispatcher.updatePolicy(sec, ptype, oldRule, newRule);
            return true;
        }

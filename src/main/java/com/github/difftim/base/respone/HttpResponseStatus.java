    ...
    public static HttpResponseStatus parseLine(CharSequence line) {
        ...
        try {
            ...
            if (space == -1) {
                ...
                return responseStatus.reasonPhrase().contentEquals(reasonPhrase) ? responseStatus : new HttpResponseStatus(code, reasonPhrase);
            }
        } catch (Exception var6) {
            throw new IllegalArgumentException("malformed status line: " + status, var6);
        }
    }


    private HttpResponseStatus(int code) {
        this(code, HttpResponseStatus.valueOf(code).reasonPhrase + " (" + code + ')', false);
    }
